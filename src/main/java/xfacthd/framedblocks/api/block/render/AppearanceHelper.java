...
public final class AppearanceHelper
{
    ...
    )
    {
        ...
        if (net.fabricmc.loader.api.FabricLoader.getInstance().getEnvironmentType() != net.fabricmc.api.EnvType.CLIENT || !type.supportsConnectedTextures())
        {
            return AIR;
        }
    ...
    )
    {
        ...
        Object renderData = rabe.getRenderAttachmentData();
        if (renderData instanceof FramedBlockData fbData)
        {
            return fbData;
        }

        if (renderData instanceof FramedDoubleBlockData doubleData)
        {
            BlockState state = level.getBlockState(pos);
            if (state.getBlock() instanceof IFramedBlock block)
            {
                Object nested = block.unpackNestedModelData(doubleData, state, componentState);
                if (nested instanceof FramedBlockData fbData)
                {
                    return fbData;
                }
            }
