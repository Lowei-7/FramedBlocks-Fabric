...
{
    public static final int STRIDE_POSITION = 0;
    public static final int STRIDE_COLOR = 3;
    public static final int STRIDE_UV0 = 4;
    public static final int STRIDE_UV2 = 6;
    public static final int STRIDE_NORMAL = 7;
    public static final int QUADS_STRIDE = 8;

    public static final Set<RenderType> SOLID = Set.of(RenderType.solid());
    public static final Set<RenderType> CUTOUT = Set.of(RenderType.cutout());
    public static final Set<RenderType> TRANSLUCENT = Set.of(RenderType.translucent());
    private static final float UV_SUBSTEP_COUNT = 8F;

    public static void unpackPosition(int[] vertexData, float[] pos, int vert)
    {
        int offset = vert * QUADS_STRIDE + STRIDE_POSITION;
    ...
    {
        int offset = vert * QUADS_STRIDE + STRIDE_UV0;
    ...
    {
        int offset = vert * QUADS_STRIDE + STRIDE_NORMAL;
    ...
    {
        int offset = vert * QUADS_STRIDE + STRIDE_COLOR;
    ...
    {
        int offset = vert * QUADS_STRIDE + STRIDE_UV2;
    ...
    public static Object getCamoModelData(Object data)
    {
        return null;
    }

    private static final MethodHandle WBM_WRAPPED_MODEL = Utils.unreflectField(WeightedBakedModel.class, "field_5435", "wrapped");

    public static List<BakedQuad> getAllCullableQuads(
            BakedModel model, BlockState state, RandomSource rand, Object data, RenderType renderType
    )
    {
        ArrayList<BakedQuad> quads = new ArrayList<>();
        for (Direction dir : Direction.values())
        {
            Utils.copyAll(getCullableQuads(model, state, rand, data, renderType, dir), quads);
        }
        return quads;
    }

    /**
     * Returns the cullable quads of the given {@link BakedModel} applicable to the given {@link Direction},
     * using quads returned for a {@code null} side (i.e. uncullable quads) if the model doesn't provide any
     * quads for the requested side
     */
    public static List<BakedQuad> getCullableQuads(
            BakedModel model, BlockState state, RandomSource rand, Object data, RenderType renderType, Direction side
    )
    {
        ...
        List<BakedQuad> sideQuads = model.getQuads(state, side, rand);
        if (sideQuads.isEmpty())
        {
            sideQuads = getFilteredNullQuads(model, state, rand, data, renderType, side);
        }
        return sideQuads;
    }
    ...
    {
        List<BakedQuad> nullQuads = model.getQuads(state, null, rand);
    ...
    public static <T> Set<T> union(Set<T> s1, Set<T> s2, Set<T> s3)
    {
        Set<T> set = new HashSet<>(s1);
        set.addAll(s2);
        set.addAll(s3);
