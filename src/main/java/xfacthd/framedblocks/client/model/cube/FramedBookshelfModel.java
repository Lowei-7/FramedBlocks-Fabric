...
public class FramedBookshelfModel extends FramedBlockModel
{
    ...
    @Override
    protected void getAdditionalQuads(Map<Direction, List<BakedQuad>> quadMap, BlockState state, RandomSource rand, Object data, RenderType renderType)
    {
        if (renderType == RenderType.cutout())
        {
            for (Direction dir : Direction.Plane.HORIZONTAL)
            {
                quadMap.get(dir).addAll(baseModel.getQuads(state, dir, rand));
            }
        }
    }
