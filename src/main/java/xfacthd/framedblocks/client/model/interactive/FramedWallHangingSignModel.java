...
public class FramedWallHangingSignModel extends FramedBlockModel
{
    ...
    protected void getAdditionalQuads(
            Map<Direction, List<BakedQuad>> quadMap, BlockState state, RandomSource rand, Object data, RenderType renderType
    )
    {
        if (renderType == RenderType.cutout())
        {
            Utils.forAllDirections(dir -> quadMap.get(dir).addAll(baseModel.getQuads(state, dir, rand)));
        }
    }
}
