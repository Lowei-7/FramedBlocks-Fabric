...
public final class CollapsibleBlockIndicatorRenderer
{
    ...
    public static boolean onBlockOutline(WorldRenderContext world, WorldRenderContext.BlockOutlineContext event)
    {
        ...
        //noinspection ConstantConditions
        BlockState state = level.getBlockState(event.blockPos());
        ...
        PoseStack poseStack = world.matrixStack();
        Vec3 offset = Vec3.atLowerCornerOf(pos).subtract(world.camera().getPosition());
        VertexConsumer builder = world.consumers().getBuffer(FramedRenderTypes.LINES_NO_DEPTH);
